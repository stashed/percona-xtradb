...
$ helm repo update
$ helm install stash-percona-xtradb-5.7-v4 appscode/stash-percona-xtradb -n kube-system --version=5.7-v4
```
...
To install the chart with the release name `stash-percona-xtradb-5.7-v4`:
...
To uninstall/delete the `stash-percona-xtradb-5.7-v4`:

```console
$ helm delete stash-percona-xtradb-5.7-v4 -n kube-system
```
...
| image.tag                 | Tag of the image that is used to backup/restore Percona XtraDB database. This is usually same as the database version it can backup. | `5.7-v4`               |
...
```console
$ helm install stash-percona-xtradb-5.7-v4 appscode/stash-percona-xtradb -n kube-system --version=5.7-v4 --set image.registry=stashed
```
...
```console
$ helm install stash-percona-xtradb-5.7-v4 appscode/stash-percona-xtradb -n kube-system --version=5.7-v4 --values values.yaml
```
