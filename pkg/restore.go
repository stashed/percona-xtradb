...
func NewCmdRestore() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			restoreOutput, err = opt.restorePerconaXtraDB(targetRef, config)
			if err != nil {
...
func (opt *perconaOptions) restorePerconaXtraDB(targetRef api_v1beta1.TargetRef, config *restclient.Config) (*restic.RestoreOutput, error) {
	var err error
	err = license.CheckLicenseEndpoint(config, licenseApiService, SupportedProducts)
	if err != nil {
		return nil, err
